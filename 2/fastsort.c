...
#define DATA_TO_INT(d) *(uint64_t*)(&d)
#define INT_TO_DATA(i) *(keyData*)(&i)
...
static const uint64_t remMul[M_W] = { M*M*M*M*M*M*M*M*M, M*M*M*M*M*M*M*M, M*M*M*M*M*M*M, M*M*M*M*M*M, M*M*M*M*M, M*M*M*M, M*M*M, M*M, M, 1 };
...
int loadFileIntoMemory(char **buffer, int *sz_buffer, char *inputFile) {
	char *mallocFailed = "malloc failed\n";
	char *statErr = "Error getting stats on file\n";
	char *loadErr = "Error loading file into memory\n";
	char *noFile = "Error: Cannot open file %s\n";
...
inline static void foundToken(cacheStack *keys, bucket *buckets, char *onl, char *start, char *end, char *fileBuffer) {
	// store in a keyData struct to save space
	keyData k;
	// TODO look up table. Make sure that you have entries for ' ' and '\0' also consider if it's one character and the first character
	// TODO make this so that it doesn't include the index
	// TODO reorder for speed
	// TODO unify indexing
	if (start == end) {
		k.ix_bucket = 0;
	} else if (start + 1 == end) {
		k.ix_bucket = IX_BUCKET1(start[0]);
		++start;
	} else {
		k.ix_bucket = IX_BUCKET2(start[0], start[1]);
		start += 2;
	}
	...
	bucket *curBucket = &buckets[k.ix_bucket];
	if (curTokenSize > curBucket->maxTokenSize) {
		curBucket->maxTokenSize = curTokenSize;
	}


	k.ix_token = start - onl; // token will always be preceeded by a newline in keys
	uint64_t key;
	...
	k.ix_nl = onl - fileBuffer;
#ifdef DEBUG
	printf("found %s ix_bucket %d ix_token %d ix_nl %d key %llu\n", start, k.ix_bucket, k.ix_token, k.ix_nl, (unsigned long long)key);
#endif
	uint64_t entry[2];
	entry[0] = DATA_TO_INT(k);
	entry[1] = key;
...
static int tokenizeLast(char * const file, cacheStack *keys, bucket *buckets) {
#ifndef PROFILE
	...
	while ((nl = strchr(onl, '\n')) != NULL) {
		...
		*nl = '\0';
		...
		// starting after the bucket ID
		foundToken(keys, buckets, onl, token, nl, file);
		onl = nl + 1;
	}
	
	return sizeStack(keys) / 2;
}

static void tokenize(char * const file, const uint_fast32_t sz_file, const int colNum, cacheStack *newLines, cacheStack *tokens, cacheStack *keys, bucket *buckets, int *numLines) {
	const char *lineTooLong = "Line too long\n";
	...
	if (sz_file > 0) {
		// TODO consider doing a look ahead to see if there are any spaces, if not, tokenize last
		onl = file;
		while ((nl = strchr(onl, '\n')) != NULL) {
#ifndef PROFILE
			if (nl - onl >= 128) {
			...
			*nl = '\0';
			...
			token = onl;
			ix_col = 1;
			char *nextTok = strchr(token, ' ') + 1;
			while (ix_col < colNum && nextTok != NULL && nextTok < nl) {
				++ix_col;
				token = nextTok;
				nextTok = strchr(token, ' ') + 1; 
			}
			// either ix_col == colNum or nextTok >= nl, so we must take a token
			foundToken(keys, buckets, onl, token, nl, file);
			onl = nl + 1;
		}
	}
...
void storeKeyInts(cacheStack *keys, bucket *buckets, int numLines) {
	...
	for (ix_line = 0; ix_line < numLines; ++ix_line) {
		uint64_t k_d = getStack(keys, ix_line * 2);
		keyData k = INT_TO_DATA(k_d);
		bucket *curBucket = &buckets[k.ix_bucket];
		token *t = &curBucket->tokens[ix_tokenBucket[k.ix_bucket]];
		t->key = getStack(keys, ix_line * 2 + 1);
		t->line = ix_line;
#ifdef DEBUG
		printf("bucket %p line %d = %d t->line\n", curBucket, ix_line, t->line);
		printf("%d: %d = %llu\n", k.ix_bucket, t->line, (unsigned long long)t->key);
		printf("idx %p\n", &ix_tokenBucket[k.ix_bucket]);
#endif
		++ix_tokenBucket[k.ix_bucket];
	}
}
...
void storeKeys(char *fileBuffer, cacheStack *keys, int numLines, bucket *buckets, int maxSz) {
	...
	for (ix_line = 0; ix_line < numLines; ix_line++) {
		...
		int ix_file = k.ix_nl + k.ix_token;
...
int main(int argc, char *argv[]) {
	...
	int numLines;
	if (colNum > MAX_COLS) {
		numLines = tokenizeLast(fileBuffer, keys, buckets);
	} else {
	...
	for (ix_bucket = 0; ix_bucket < NUM_BUCKETS; ++ix_bucket) {
		...
		if (curBucket->numTokens > 0) {
			...
			// TODO replace with my own sort that includes networks
			...
			// TODO resort only this bucket, the rest should all be fine
			if (resort == 1) {
				break;
			}
#endif
